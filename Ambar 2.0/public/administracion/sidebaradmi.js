// sidebaradmi.js — Barra lateral del Portal Administrador (sistema Ámbar)
(function () {
    "use strict";
    ...
    const html = `
        <style>
            ...
            #sidebar-container a.nav-item:hover { background: rgba(255,255,255,.07); color: white; }
            #sidebar-container a.nav-item.active {
                background: rgba(232,161,0,.12); border-left: 3px solid #E8A100; color: #E8A100;
            }
            #sidebar-container .nav-icon { font-size: 18px; width: 22px; text-align: center; flex-shrink: 0; }
            #sidebar-container .nav-bottom-wrap { border-top: 1px solid rgba(255,255,255,.07); padding: 8px 0; }
            #sidebar-container .nav-main { flex: 1; }
        </style>
        <div class="nav-section-label">Menú principal</div>
        <div class="nav-main">${enlacesPrincipales.map(buildItem).join('')}</div>
        <div class="nav-bottom-wrap">${enlacesInferiores.map(buildItem).join('')}</div>
    `;
